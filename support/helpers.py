...
def testinfo_payload(text):  # noqa: E302
...
#? handle the case exception PIL.UnidentifiedImageError: cannot identify image file
# por ejemplo en una corrida todas los files eran de tamaño 0
...
def update_snapshots(db, filename_persist, target_scripts, snapshots_dir, py_cmd=None):
    """
    ...
    proxy_abspath = os.path.join(module_dir, 'proxy_snapshots.py')
    ...
    for script in valid_scripts:
        ...
        missing = [s for s in expected_snapshots
        ...
        if not snapshots_success:
            if timeout_hit:
                err = ('Timeout hit. Remember in this case it is posible not' +
                       ' all stderr captured.\n' +
                       err)
            if len(missing):
...
def measure_repeteability(db, candidates, limit, samples_dir, required_md5=None):
    ...
    for name in stats_by_script_name:
        hasher.update(asciibytes(db.get_prop_value(name, 'md5_at_testinfo')))
    ...
    while f_continue():
        for name in scripts:
            ...
            timeout_hit, err = proxy_run(proxy_abspath,
                                         fname,
                                         [stored_testinfo, snapshots_abspath])
            # count errors
            if timeout_hit:
...
def fn_allow_testrun_pass(propdict):
    return ('testrun_success' in propdict and
...
def section(db, selector, candidates=None, verbose=None, testbed=None):
    headers = {
        ...
        'testrun_not_pass': "Scripts whith valid testinfo and don't have testrun_success=='pass'",
...
def progress_report(db, verbose=False):
    ...
    text = rpt(db, selectors, verbose=verbose)
    return text
